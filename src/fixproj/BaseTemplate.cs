...
namespace FixProjects
{
    public class BaseTemplate
    {
        ...
        protected BaseTemplate(string filePath)
        {
            FilePath = filePath;
        }
        ...
        /// <summary>
        ///     Gets or sets the last inserted node.
        /// </summary>
        protected XNode InsertedAt { get; set; }

        /// <summary>
        ///     Gets or sets the collection of ItemGroup elements.
        /// </summary>
        ...
        protected void DeleteDuplicatesBasedOnAttribute(ItemGroupEntity entity, IList<string> changes,
            Func<XElement, string> func)
        {
            entity.Element
                ...
                .Select(
                    g =>
                    {
                        ...
                        changes.Add(
                            $"{entity.LocalName}: removed {dupesOfFirst.Count} dupes of {dupesOfFirst.First().Attribute("Include")?.Value}");
        ...
        {
            if (element?.Value.Contains(Constants.AlwaysNodeValue) != true) return;

            element.Value = Constants.PreserveNewestNodeValue;
            changes.Add(
                $"{element.Name.LocalName}: Changing {element.AttributeValueByName(Constants.IncludeAttribute)} to PreserveNewest instead of CopyAlways");
        }
        ...
        protected void FixPropertyGroups(XElement element, IList<string> changes,
            IList<string> propertiesToDelete = null)
        {
            element.ElementsByLocalName(Constants.PropertyGroupNode)
                .ForEach(
                    property =>
                    {
                        ...
                        if (propertiesToDelete == null) return;
